...
Report <- R6Class(
  ...
  public = list(
    ...
    printExamples = function() {
      ...
      o$obfuscate(p$patientIdentifiers,
                'PATIENT ID')
      self$NL()
      writeLines(paste0(
        basename(p$reportParams$dataFile),
      ...
      o$obfuscate(p$patientNameChanges,
                'name changed')
      self$NL()
      writeLines(paste0(
        basename(p$reportParams$dataFile),
      ...
      o$obfuscate(p$surgicalNumbers, 'SURGICAL #')
      self$NL()
      writeLines(paste0(
        basename(p$reportParams$dataFile),
        "\t",
      ...
      o$obfuscate(p$patientDateOfBirth,
                'date of birth')
      self$NL()
      
      
      self$NL()
      if (nrow(p$patientAgeOverMax) > 0) {
        writeLines(paste0(
          basename(p$reportParams$dataFile),
          "\t",
        ...
        self$NL()
        for (i in 1:nrow(head(p$patientAgeOverMax, n = p$reportParams$instancesToShow))) {
          row <- p$patientAgeOverMax[i, ]
          writeLines(paste0(
            "Row:",
            "\t",
      ...
      if (nrow(p$validatedZipCodes) > 0)
      {
        ...
        self$NL()
        for (i in 1:nrow(head(p$validatedZipCodes, n = p$reportParams$instancesToShow))) {
          row <- p$validatedZipCodes[i, ]
          writeLines(paste0(
            "Row:",
            "\t",
            as.integer(row$rownumber) + 1,
      ...
      if (nrow(p$validPatDrawDates) > 0)
      {
        ...
        self$NL()
        for (i in 1:nrow(head(p$validPatDrawDates, n = p$reportParams$instancesToShow))) {
          row <- p$validPatDrawDates[i, ]
          writeLines(
            paste0(
              "Row:",
              "\t",
